...
const connectors = {
  ...
  weather: require('./connectors/weatherConnector'), // ÆGTE data, statisk snapshot
...
app.get('/api/weather', async (req, res) => {
  try {
    res.json({ fetchedAt: connectors.weather.fetchedAt, data: await connectors.weather.fetchAll() });
  } catch (err) {
    res.status(500).json({ error: 'Kunne ikke hente vejrdata', detaljer: err.message });
  }
});
...
const AUTO_REFRESH_CONNECTORS = ['earnings', 'weather', 'news', 'market_events', 'ships'];
...
cron.schedule('0 6 * * *', runAutoRefresh);

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => {
  console.log(`Data platform running on http://localhost:${PORT}`);
  console.log('Automatisk dagligt opdateringsjob aktivt (kl. 06:00) — se AUTO_REFRESH_CONNECTORS i server.js');
